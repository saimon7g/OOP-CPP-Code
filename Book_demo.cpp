...
class Book{
	...
	public:
		...
		Book(int isbn, char* title, int price){//constructor with parameters
			...
			this->price=price;
			cout<< green<<"In the parameterized constructor of the book: "<<isbn<<endl;
		}
		Book(const Book& book){//Copy constructor
			...
			cout<< green<<"In the copy constructor of the book: "<<isbn<<endl;
		}

		//destructor function
		~Book(){
			cout<< red <<"In the destructor of the book: "<<isbn<<endl ;
			delete[] title;
